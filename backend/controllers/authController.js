...
exports.signup = async (req, res) => {
  try {
    ...
    if (existing) {
      const msg = existing.googleId
        ? 'This email is linked to a Google account. Please sign in with the Google button.'
    ...
    if (!passwordOk) {
      if (user.googleId) {
        return res.status(401).json({
          success: false,
          message: 'This account was created with Google Sign-In. Please use the "Continue with Google" button or the OTP tab to log in.',
